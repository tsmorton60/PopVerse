...
function saveTypeOptions() {
	...
	for (var i = 0; i < radios.length; i++) {
		if (radios[i].checked) {
			chrome.storage.local.set({linkType: radios[i].value});
			break;
		}
	}
}
...
function saveColorOptions() {
	...
	for (var i = 0; i < radios.length; i++) {
		if (radios[i].checked) {
			chrome.storage.local.set({colorTheme: radios[i].value});
			break;
		}
	}
}

function saveCtrlOptions() {
	var radios = document.getElementsByName('ctrl1');
	for (var i = 0; i < radios.length; i++) {
		if (radios[i].checked) {
			chrome.storage.local.set({ctrlOpt: radios[i].value});
			break;
		}
	}
}
...
function titleInsert() {
	...
	console.log(isEnabled);
	chrome.storage.local.set({titleInsert: isEnabled});
}
